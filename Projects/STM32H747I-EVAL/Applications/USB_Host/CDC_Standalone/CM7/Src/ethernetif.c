...
//  ethernet_link_check_state(netif, noti_buf, size);
}
...
//      p = pbuf_alloced_custom(PBUF_RAW, framelength, PBUF_REF, custom_pbuf, RxBuff->buffer, framelength);
      p = pbuf_alloced_custom(PBUF_RAW, size_rx_last, PBUF_REF, custom_pbuf, rx_buffer_store, size_rx_last);
    }
...
CDC_ECM_LinkStateTypeDef;

CDC_ECM_LinkStateTypeDef prevPHYLinkState = LINK_DISCONNECT;

CDC_ECM_LinkStateTypeDef parse_connection_speed_change(uint8_t *noti_buf, uint8_t size){
	uint32_t dlink_speed = 0;
	CDC_ECM_LinkStateTypeDef PHYLinkState = LINK_DISCONNECT;

	if (size < 1)
	{
		PHYLinkState = prevPHYLinkState;
	} else if (noti_buf && size > 0 && size == 8){
		if(noti_buf[0] == 0xa1 && noti_buf[1] == 0x00 && noti_buf[2] == 0x01)
		{
			PHYLinkState = LINKED;
		}
	} else if (noti_buf && size > 0 && size == 16){
		//check if first byte is a1
		if(noti_buf[0] == 0xa1 && noti_buf[1] == 0x2a)
		{
			//start at index 8, since first 8 bytes don't contain link speed data
			//example
			//A1 2A 00 00 01 00 08 00 00 E1 F5 05 00 E1 F5 05
			int j = 3;
			for (int i = 8; i < size-4; i++)
			{
				dlink_speed |= (noti_buf[i] << j*8);
				j--;
			}

			if (dlink_speed > 10000000 && dlink_speed < 100000000)
			{
				PHYLinkState = LINK_10M;
			}

			if (dlink_speed > 100000000)
			{
				PHYLinkState = LINK_100M;
			}
		}
	}
...
/**
  ...
  * @retval None
  */
void ethernet_link_check_state(struct netif *netif, uint8_t *noti_buf, uint8_t size)
{
//	ETH_MACConfigTypeDef MACConf;
	CDC_ECM_LinkStateTypeDef PHYLinkState;
	uint32_t linkchanged = 0;

//	if (noti_buf && size > 0 && size == 16){
//		//check if first byte is a1
//		if(noti_buf[0] == 0xa1)
//		{
//			if(noti_buf[2] == 1)
...
//	}
	...
	PHYLinkState = parse_connection_speed_change(noti_buf, size);

	if(netif_is_link_up(netif) && (PHYLinkState <= LINK_DISCONNECT))
	{
//		HAL_ETH_Stop(&EthHandle);
		netif_set_down(netif);
		netif_set_link_down(netif);
	}
	else if(!netif_is_link_up(netif) && (PHYLinkState > LINK_DISCONNECT))
	{
		switch (PHYLinkState)
		{
		case LINK_10M:
